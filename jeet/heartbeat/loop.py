...
class Heartbeat:
    ...
    def _run(self):
        while not self._stop.is_set():
            ...
            # The kill switch pauses without tearing anything down: keep
            # polling (so it notices the moment it's un-paused) but don't
            # run any checks while paused. Timers already in flight — the
            # per-check next_due in state.py — are untouched, so pausing
            # never resets a schedule.
            if not killswitch.is_paused():
                for check_cfg in config.get("checks", []):
                    ...
                    # Mark the next run before starting this one: even if
                    # the check hangs or crashes, the schedule has already
                    # moved forward, so a stuck check can't spin the loop.
    ...
    def _run_check_safely(self, name, params):
        ...
        try:
            ...
            audit.log_heartbeat_error(name, e)
